...
public sealed class Resource
{
    ...
    private Resource(Key key, string name, ResourceType type, Unit unit, Money price)
    {
        ...
        if (string.IsNullOrWhiteSpace(name)) throw new DomainException("Resource name is required.");
        Name = name.Trim();
        Type = type;
        Price = price;
    ...
    public static Resource Create(Key key, string name, ResourceType type, Unit unit, Money price) => new(key, name, type, unit, price);
    public void Rename(string name) { if (string.IsNullOrWhiteSpace(name)) throw new DomainException("Resource name is required."); Name = name.Trim(); }
